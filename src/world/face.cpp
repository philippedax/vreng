...
const OClass Face::oclass(FACE_TYPE, "Face", Face::creator);

//local
static Face *phead = NULL;
...
uint8_t Face::getGender(const char *name)
{
  if (name) {
    for (struct sFace *pheads = heads ; pheads; pheads++) {
...
Face::Face(User *user, void *d, time_t s, time_t u)
{
...
Face::Face(User *user, const char *url, const float *skin)
{
  if (! url) return;
...
void Face::changePermanent(float lasting)
{
  pos.x = localuser->pos.x + dx;
  pos.y = localuser->pos.y + dy;
  switch (model_t) {
...
void Face::visibility(bool flag)
{
  visible = flag;
}
...
void Face::recreate_cb(User *user, void *d, time_t s, time_t u)
{
  new Face(user, d, s, u);
}
...
void Face::funcs()
{
  setActionFunc(FACE_TYPE, RECREATE, _Action recreate_cb, "");
}
