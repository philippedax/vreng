...
void FTGLOutlineFont::Render( const char* string, int len, float x, float y)
{   
    ...
    FTFont::Render( string, len, x, y);

    glPopAttrib();
}


void FTGLOutlineFont::Render( const wchar_t* string, int len, float x, float y)
{   
    glPushAttrib( GL_ENABLE_BIT | GL_HINT_BIT | GL_LINE_BIT | GL_COLOR_BUFFER_BIT);
