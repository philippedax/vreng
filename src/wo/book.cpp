...
void Book::parser(char *l)
{
  ...
  begin_while_parse(l) {
    ...
    else if (!stringcmp(l, "aleft"))          l = parse()->parseFloat(l, &aleft, "aleft");
    else if (!stringcmp(l, "nbsheets"))       l = parse()->parseUInt8(l, &nbs, "nbsheets");
    else if (!stringcmp(l, "sheetthickness")) l = parse()->parseFloat(l, &thick, "sheetthickness");
    else if (!stringcmp(l, "currentsheet"))   l = parse()->parseUInt8(l, &num, "currentsheet");
  }
  end_while_parse(l);
}
