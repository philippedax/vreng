...
extern int yyparse();  // parseur créé par bison


char vrelfile[100];	// fichier lu courant
int comptfichier;	// numero de fichier ouvert
FILE * tabyyin[20];	// tableau mémorisant le pointeur sur le fichier ouvert 
int tablinenumber[20];	// tableau mémorisant le numéro de ligne dans ce fichier
...
int main(int argc, char *argv[]) 
{
  ...
  fprintf(yyout, "<!-- File generated by %s -->\n<vre version=\"7.2\">\n<head>\n<meta name=\"filename\" content=\"%s\"/>\n</head>\n<scene>\n",
          argv[0], (argc == 3) ? argv[2] : "stdin");
  yyparse();
  prog->exec();
  fprintf(yyout, "</scene>\n</vre>\n");
