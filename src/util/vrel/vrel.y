...
PROGRAMME : VIDE
	...
	| PROGRAMME DECLARATION_VAR
                { $1->adddeclarvar($2) ;  /* Déclaration de variable globale effectuée */ }
...
DECLARATION_VAR : VAR VARIABLE PTVIRG
		{ $$ = new Decl_var($2) ; /* Déclaration simple */ }
	| VAR VARIABLE EGAL DATA PTVIRG
		{ $$ = new Decl_var($2, $4) ; /* Déclaration et affectation */ } 

DECLARATION_VAR_LISTE : VIDE
		{ $$ = new Decl_var_liste() ; /* Début des déclarations de variables */ }
	| DECLARATION_VAR_LISTE DECLARATION_VAR
		{ $1->adddeclarvar($2) ;  /* Déclaration de variable effectuée */ }

DECLARATION_CLASSE : CLASSE NOM EXTENDS NOM ACCOUVR AFFECTATION_CLASSE_LISTE ACCFERM 
		{ $$ = new Class_extends($2, $4, $6) ; /* Classe héritée */ }
...
DEFINITION_CLASSE : VIDE
		{ $$ = new Env_constr() ; /* Début de la définition de la classe */ }
        | DEFINITION_CLASSE CONSTRUCTEUR
                { $1->addConstr($2) ; /* Nouveau constructeur rentré */ }
...
INSTRUCTION_LISTE : VIDE
		{ $$ = new Instruction_liste() ; /* Début des instructions */ }
...
INSTRUCTION : AFFECTATION
	...
	| VARIABLE EGAL NEW NOM PARENTOUVR DATALISTE PARENTFERM PTVIRG
		{ $$ = new Creation($1, $4, $6) ; /* Nouvel objet créé */ }
...
CONSEQUENCE : ACCOUVR INSTRUCTION_LISTE ACCFERM
		{ $$ = $2 ; /* Fin des conséquences */ }

AFFECTATION : VARIABLE EGAL DATA PTVIRG
		{ $$ = new Affectation($1, $3) ; /* Affectation effectuée */ }
         | NOM POINT NOM EGAL DATA PTVIRG
		{ $$ = new Affectation($1, $3, $5) ; /* Elément d'un objet */ }
...
COMPARAISON : DATA EGALEGAL DATA
	...
	| DATA SUP DATA
		{ $$ = new Superieur($1, $3) ; /* Supérieur */ }
	| DATA INF DATA
		{ $$ = new Inferieur($1, $3) ; /* Inférieur */ }
	| DATA SUPEGAL DATA
		{ $$ = new Supegal  ($1, $3) ; /* Supérieur ou Egal */ }
	| DATA INFEGAL DATA
		{ $$ = new Infegal  ($1, $3) ; /* Inférieur ou Egal */ }
	| DATA DIFFERENT DATA
		{ $$ = new Different($1, $3) ; /* Différent */ }
